...
import { getAdminProjects } from "@/services/project.server-service";

export default async function AdminProjectsPage() {
  await requireAdmin();
  const response = await getAdminProjects(50);
                  ...
                  <tr key={project.id} className="border-t border-[color:var(--border)]">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-2">
                        <Link className="inline-flex min-h-9 items-center gap-2 rounded-full border border-[color:var(--border)] px-3 text-xs font-bold hover:border-[color:var(--primary)]" href={`/dashboard/admin/projects/${project.id}/edit`}><FiEdit3 />Edit</Link>
